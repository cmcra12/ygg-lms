// Scale seed — ~1600 fake deals on top of the base seed, to test how the LMS
// handles a full book. Run after a normal seed:
//
//   npm run db:reset && npm run db:seed-scale
//
// Every generated row gets an explicit id in the 100001+ range (everything
// below that is left for the base seed and live data), so the exact same rows
// are also written out as additive SQL files (supabase-scale-deals-*.sql) that
// can be pasted into the Supabase SQL Editor to load the same book into the
// live database without disturbing anything already in it.
//
// The generator is seeded, so re-running always produces the same book.
...
import { isValidAbn, isValidAcn } from "../src/lib/abn";
import { DEFAULT_CHECKLIST } from "../src/lib/checklist";
import { WORKFLOW_TEMPLATES } from "../src/lib/workflows";

const DEALS_WANTED = Number(process.env.SCALE_DEALS ?? 1600);
const ID_BASE = 100000; // scale rows live above this in every table
...
const stampOf = (d: Date) => new Date(d.getFullYear(), d.getMonth(), d.getDate(), 9, rint(0, 59)).toISOString();
...
const makeSerial = (make: string) => `${make.toUpperCase().replace(/[^A-Z0-9]/g, "").slice(0, 3)}-${serialSeq++}`;

// --- Row store: everything is generated in memory first ------------------------
...
const seq = Object.fromEntries(TABLE_ORDER.map((k) => [k, ID_BASE])) as unknown as Record<TableKey, number>;
...
async function main() {
  const already = (await db.select({ id: t.loans.id }).from(t.loans)).some((r) => r.id > ID_BASE);
  if (already) {
    console.log("Scale data already present (loan ids above 100000) — run `npm run db:reset` first to regenerate.");
    await closeDb();
    return;
  }
  ...
  let customerCount = 0;
  while (dealSpecs.length < DEALS_WANTED) {
    ...
    for (let d = 0; d < dealCount && dealSpecs.length < DEALS_WANTED; d++) {
  ...
  for (const [i, spec] of dealSpecs.entries()) {
    ...
    for (const dealAsset of dealAssets) {
      ...
      const registrationId = add("ppsrRegistrations", {
        ...
        expiryDate: status === "active" ? isoDate(new Date(start.getFullYear() + 7, start.getMonth(), start.getDate())) : endDate ?? startDate,
    ...
    if (arrears) arrearsLoans.push({ loanId, openedAt: stampOf(new Date(TODAY.getFullYear(), TODAY.getMonth(), TODAY.getDate() - rint(3, 40))) });
  }
  ...
  // --- Write the additive Supabase SQL files -------------------------------------
  ...
  const header = (part: number, total: number) => `-- YGG LMS — ${DEALS_WANTED} fake deals for scale testing (part ${part} of ${total})
...
-- generated rows use ids above ${ID_BASE}. Re-running a part that already
...
`;
  ...
  for (const key of TABLE_ORDER) {
    const table = DRIZZLE_TABLES[key];
    const { name: tableName } = getTableConfig(table);
    const columns = getTableColumns(table);
    const keys = ["id", ...Object.keys(columns).filter((k) => k !== "id")];
    const columnNames = keys.map((k) => `"${columns[k as keyof typeof columns].name}"`).join(", ");
    const rows = store[key];
    for (let i = 0; i < rows.length; i += 250) {
      const values = rows
        ...
        .join(",\n");
      statements.push(`INSERT INTO "${tableName}" (${columnNames}) OVERRIDING SYSTEM VALUE VALUES\n${values};`);
    }
  }
  ...
  for (const statement of statements) {
    if (size + statement.length > MAX_PART_BYTES && parts[parts.length - 1].length > 0) {
      parts.push([]);
      size = 0;
    }
    parts[parts.length - 1].push(statement);
    size += statement.length;
  }
  for (const old of fs.readdirSync(".").filter((f) => /^supabase-scale-deals-part\d+\.sql$/.test(f))) {
    fs.unlinkSync(old);
  }
  for (const [i, part] of parts.entries()) {
    fs.writeFileSync(
      `supabase-scale-deals-part${i + 1}.sql`,
      header(i + 1, parts.length) + "\n" + part.join("\n") + "\n",
    );
  }

  const counts = Object.fromEntries(TABLE_ORDER.map((k) => [k, store[k].length]));
  console.log("Scale seed complete:", counts);
  console.log(
    `Deals: ${dealSpecs.length} (${statusTally.active} active, ${statusTally.paid_out} paid out, ${statusTally.written_off} written off, ${arrearsCount} in arrears)`,
  );
  console.log(`Contracts ${`YGG${CONTRACT_START}`}–YGG${CONTRACT_START + dealSpecs.length - 1}, customers ${customerCount}`);
  console.log(`Wrote ${parts.length} SQL part file(s): supabase-scale-deals-part1..${parts.length}.sql`);
  await closeDb();
}
